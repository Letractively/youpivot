...
(function(){
	...
	function addToDomainList(domain, color, data){
        ...
        newDomain.graphColor = [1, 255].concat(Color.toRGBArray(graphColor));
	...
	function addItem(id, item){
		...
		if(importance && importance.length>0){ 
			var data = GraphManager.addLayer(item.domain.color, item.importance, item.id, item.startTime, domain.name);
            item.domain.id = addToDomainList(domain.name, domain.color, data);
		}else{
