...
var TMModel = new (function _TMModel(){
   var self = this;
   
   var timemarks = [];
   
   self.printTimeMarks = function _getTimemarks(){ 
      
      Database.runTransaction(function(tx){
         tx.executeSql("SELECT * FROM timemark ORDER BY timestamp DESC",[],processTimeMarks);
      });
   }
   
   self.toggleTimemarksEdit = function _toggleTimemarksEdit(){
   ...
   self.deleteTimeMarkPage = function _deleteTimeMarkPage(id){
      Database.runTransaction(function(tx){
   ...
   self.deleteTimeMark = function _deleteTimeMark(id){
      Database.runTransaction(function(tx){
   ...
   self.reopenTimemark = function _reopenTimemark(id){
   ...
   self.openPageBox = function openPageBox(id){
      $(id).toggleClass("open");
   }
   
   function processTimeMarks(tx,result){
   ...
   function printTimeMark(item,rows){
     var string = "<div class='entry' id='timemark-"+item.id+"' data-color='"+item.color+"'>";
     var timestamp = new Date(item.timestamp);
     string += "<div class='timeMarkHeader'>"
     string += "<div class='time'>" + timestamp.format("shortTime") + "</div>";
     string += "<a onclick=\"TMModel.openPageBox('#timemark-"+ item.id  +"');\" class='description'><img class='timeMarkIcon' src='images/timeMarkMarks/timemark_32_" + item.color  + ".png' width='16' height='16' />";
     ...
     string += " <span class='deletePage'><a href='javascript:TMModel.deleteTimeMark(\"" + item.id + "\");'>Delete</a></span>";
     ...
     for(i=0;i<rows.length;i++){
        ...
        if(item.window != currentWindow){
         ...
         string += " <span class='deletePage'><a href='javascript:TMModel.deleteTimeMarkPage(\"" + item.id + "\");'>Delete</a></span>";
         string += "</div>";
      }
      string += "</div>";
      string += "</div>";
