...
database.createTable = function() {
   if(database.db.version == ''){
      database.db.changeVersion("","0.0.7",function(){
         database.db.transaction(function(tx) {
            ...
            tx.executeSql("create table location(id string, title string, latitude integer, longitude integer, radius integer)",[]);
         });
      });
   }
   ...
   if(database.db.version == "0.0.6"){
       database.db.changeVersion("0.0.6","0.0.7",function(){
          database.db.transaction(
             function(tx) {
                tx.executeSql("create table location(id string, title string, latitude integer, longitude integer, radius integer)",[]);
             }
